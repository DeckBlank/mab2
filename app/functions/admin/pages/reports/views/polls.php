<div 
    ...
    <h2 class="mb-1">Encuestas respondidas</h2>
    <div class="mb-1">
    </div>
    <div class="d-flex mb-1">
        ...
        <a  
            href="<?php echo get_site_url() . '/wp-json/custom/v1/behaviour/polls/download' ?>" 
            download 
...
<script>
    ...
    function getResults(__page){
        fetch(`${API}/behaviour/polls?page=${__page}`)
            .then(res => {
    ...
    function searchUserResult(){
        ...
        fetch(`${API}/behaviour/polls?user=${user.value}`)
            .then(res => {
                if (res.status >= 200 && res.status < 300) {
                    return res.json()
                }else{
