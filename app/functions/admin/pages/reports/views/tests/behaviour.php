<div 
    ...
    <h2 class="mb-1">Estilos de personalidad</h2>
    <div class="mb-1">
    </div>
    <div class="d-flex mb-1">
        ...
        <a  
            href="<?php echo get_site_url() . '/wp-json/custom/v1/tests/behaviour/download' ?>" 
            download 
    ...
    <table class="widefat fixed mb-1" cellspacing="0">
        <thead>
            <tr>
                ...
                <th id="columnname" class="manage-column column-columnname" scope="col">Perfil</th>
...
<script>
    ...
    function mountResults(__logs, reset){
        ...
        __logs.forEach((log, index)=>{
            ...
            <tr valign="top" class="${ ((index + 1) % 2 == 0) ? 'alternate' : '' }">
                ...
                <td class="manage-column column-columnname" scope="col">${result.title}</td>
    ...
    function getResults(__page){
        fetch(`${API}/tests/behaviour?page=${__page}`)
            .then(res => {
    ...
    function searchUserResult(){
        ...
        fetch(`${API}/tests/behaviour?user=${user.value}`)
            .then(res => {
                if (res.status >= 200 && res.status < 300) {
                    return res.json()
                }else{
