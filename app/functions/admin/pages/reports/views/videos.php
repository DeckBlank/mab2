<div 
    ...
    <h2 class="mb-1">Videos vistos</h2>
    <div class="mb-1">
    </div>
    <div class="d-flex mb-1">
        ...
        <a 
            href="<?php echo get_site_url() . '/wp-json/custom/v1/topics/video/logs/download' ?>" 
            download 
...
<script>
    ...
    function mountResults(__logs, reset){
        ...
        __logs.forEach((log, index)=>{
            ...
            <tr valign="top" class="${ ((index + 1) % 2 == 0) ? 'alternate' : '' }">
                ...
                <td class="manage-column column-columnname" scope="col">${log.views}</td>
    ...
    function getResults(__page){
        fetch(`${API}/topics/video/logs?page=${__page}`)
            .then(res => {
    ...
    function searchUserResult(){
        ...
        fetch(`${API}/topics/video/logs?user=${user.value}`)
            .then(res => {
                if (res.status >= 200 && res.status < 300) {
                    return res.json()
                }else{
