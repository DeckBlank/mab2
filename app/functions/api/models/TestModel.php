...
class TestModel{
    ...
    public static function saveBehaviourTest($request){
        return DBConnection::getConnection()->query("
            INSERT INTO wp_user_behaviour_tests(date_at,user,result) VALUES(
                '". date("Y-m-d") ."',
                '". $request['user'] ."',
                '". $request['result'] ."'
            )
    ...
    public static function getBehaviourTest($request){
        $tests = DBConnection::getConnection()->query("
            SELECT * 
            FROM
                wp_user_behaviour_tests
            WHERE
                user = '". $request['user'] ."'
        ");
        $test_results = [];
    ...
    public static function getBehaviourTests($request, $limit = false){
        if (isset($request['user'])) {
            $test_logs_query = DBConnection::getConnection()->query("
                SELECT 
                    *
